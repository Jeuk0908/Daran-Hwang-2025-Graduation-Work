 ...
 * @param {string} props.targetProfit - 목표 수익 금액
 * @param {boolean} props.isProfitMode - 수익 모드 여부 (true: 순 수익, false: 남은 목표 수익)
 * @param {function} props.onProfitModeToggle - 수익/목표 토글 핸들러
...
export const RebalanceInfoCard = ({
  ...
  const calculateDisplayAmount = () => {
    if (isProfitMode) {
      ...
      // 목표 수익 - 현재 순 수익
      const targetNum = parseInt(targetProfit.replace(/,/g, ''));
      const netNum = parseInt(netProfit.replace(/,/g, ''));
      const remaining = targetNum - netNum;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 자세히 모드일 때만 표시: 설정 변화율 + 리밸런싱 필요 여부 */}
        <div
          style={{
        ...
        >
          ...
          >
            ...
            {/* 자세히 모드일 때 "리밸런싱을 하는 이유?" 표시 */}
            <RebalanceReasonSection />
          </div>
        </div>

        {/* 간략히 모드일 때만 "리밸런싱을 하는 이유?" 별도로 표시 */}
        {!isExpanded && (
          <div
            style={{
              alignSelf: 'stretch',
